...
from apps.business_central.helpers import (
...
def test_check_interval_and_sync_dimension(
    ...
    business_central_creds = BusinessCentralCredentials.objects.get(workspace_id=workspace_id)
    workspace = Workspace.objects.get(id=workspace_id)

    workspace.destination_synced_at = None
    workspace.save()

    sync_dimensions_mock = mocker.patch('apps.business_central.helpers.sync_dimensions', return_value=None)

    return_value = check_interval_and_sync_dimension(
        workspace=workspace,
        business_central_credential=business_central_creds
    )

    assert sync_dimensions_mock.call_count == 1
    ...
    assert sync_dimensions_mock.call_count == 2
    assert return_value == True
...
def test_sync_dimensions(
    ...
    business_central_creds = BusinessCentralCredentials.objects.get(workspace_id=workspace_id)
    business_central_connection_mock = mocker.patch('apps.business_central.utils.BusinessCentralConnector')

    mock_sync_companies = mocker.patch.object(
        business_central_connection_mock.return_value, 'sync_companies'
    )
    mock_sync_accounts = mocker.patch.object(
        business_central_connection_mock.return_value, 'sync_accounts'
    )
    mock_sync_vendors = mocker.patch.object(
        business_central_connection_mock.return_value, 'sync_vendors'
    )
    mock_sync_employees = mocker.patch.object(
        business_central_connection_mock.return_value, 'sync_employees'
    )
    mock_sync_locations = mocker.patch.object(
        business_central_connection_mock.return_value, 'sync_locations'
    )
    ...
    assert mock_sync_accounts.call_count == 1
    assert mock_sync_vendors.call_count == 1
    assert mock_sync_employees.call_count == 1
    assert mock_sync_locations.call_count == 1

    business_central_connection_mock.return_value.sync_companies.side_effect = Exception('Error')
