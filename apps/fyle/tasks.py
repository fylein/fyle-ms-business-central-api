...
from apps.workspaces.models import ExportSetting, FyleCredential, LastExportDetail, Workspace
...
def re_run_skip_export_rule(workspace: Workspace) -> None:
    ...
    if expense_filters:
        ...
        if skipped_expenses:
            accounting_exports = AccountingExport.objects.filter(workspace_id=workspace.id, status__in=['EXPORT_READY', 'FAILED', 'FATAL'])
            deleted_failed_expense_groups_count = 0
            for accounting_export in accounting_exports:
                accounting_export = AccountingExport.objects.filter(
                    workspace_id=workspace.id,
                    expense_group_id=accounting_export.expense_group_id
                ).first()
                if accounting_export:
                    if accounting_export.status != 'COMPLETE':
                        deleted_failed_expense_groups_count += 1
                    logger.info('Deleting task log for expense group %s before export', accounting_export.id)
                    accounting_export.delete()

                error = Error.objects.filter(
                    workspace_id=workspace.id,
                    accounting_export__id=accounting_export.id,
                    type__in=['QBO_ERROR']
                ).first()
                if error:
                    logger.info('Deleting QBO error for expense group %s before export', accounting_export.id)
                    error.delete()

                accounting_export.expenses.remove(*skipped_expenses)
                if not accounting_export.expenses.exists():
                    logger.info('Deleting empty expense group %s before export', accounting_export.id)
                    accounting_export.delete()
