...
BEGIN
  ...
  DELETE
  FROM journal_entries_lineitems jel
  WHERE jel.workspace_id = _workspace_id;
  GET DIAGNOSTICS rcount = ROW_COUNT;
  RAISE NOTICE 'Deleted % journal_entries_lineitems', rcount;
  ...
  DELETE
  FROM business_central_credentials bcc
  WHERE bcc.workspace_id = _workspace_id;
  GET DIAGNOSTICS rcount = ROW_COUNT;
